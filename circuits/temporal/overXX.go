...
type Circuit struct {
	...
	// ClaimPosition is the position where the claim name appears within DateB64
	// This is secret to maintain privacy of the credential structure
	ClaimPosition frontend.Variable `gnark:",secret"`
...
// In this circuit, we prove: "I know a Payload containing a date claim such that:
//  1. DateB64 is a substring of Payload at position DateB64Position
//  2. Claim is a substring of DateB64 at position ClaimPosition (verifying correct field)
//  3. When DateB64 is base64url-decoded, it contains a date at DatePosition
//  4. That date is lexicographically less than ThresholdDate (proving age eligibility)"
//
// Inputs except ThresholdDate and Claim are secret, providing strong privacy.
func (c *Circuit) Define(api frontend.API) error {
	...
	// Step 2: Decode the base64url-encoded date claim FIRST
	// This extracts the JSON containing the date field
	dateJSON, err := zkcore.DecodeBase64Url(api, c.DateB64[:])
	if err != nil {
		return fmt.Errorf("failed to decode DateB64: %w", err)
	...
	// Step 3: Verify that the Claim name is embedded in DateB64 at the specified position
	// This ensures we're validating the correct field (e.g., "birthdate" not "issue_date")
	// The verifier specifies which claim to check, and we prove it exists in our date claim
	zkcore.AssertIsSubsetWithPadding(api, dateJSON, c.DateSize, c.Claim[:], c.ClaimSize, c.ClaimPosition)
...
type WitnessInput struct {
	...
	DateB64Position int    // Position of DateB64 in Payload
	DatePosition    int    // Position of date string within decoded DateB64
	ClaimPosition   int    // Position of claim name within DateB64
...
func (w *WitnessInput) Validate() error {
	...
	// Validate DateB64Position
	if w.DateB64Position < 0 || w.DateB64Position > len(w.Payload)-len(w.DateB64) {
		return fmt.Errorf("DateB64Position out of range: %d (payload length: %d, DateB64 length: %d)",
			w.DateB64Position, len(w.Payload), len(w.DateB64))
	}

	// Verify DateB64 is actually at the claimed position in payload
	if !strings.Contains(w.Payload[w.DateB64Position:w.DateB64Position+len(w.DateB64)], w.DateB64) {
		return fmt.Errorf("DateB64 not found at claimed position %d in payload", w.DateB64Position)
	}

	// Validate ClaimPosition
	if w.ClaimPosition < 0 || w.ClaimPosition > len(w.DateB64)-len(w.Claim) {
		return fmt.Errorf("claimPosition out of range: %d (DateB64 length: %d, Claim length: %d)",
			w.ClaimPosition, len(w.DateB64), len(w.Claim))
	}

	// Verify Claim is actually at the claimed position in DateB64
	if !strings.Contains(w.DateB64[w.ClaimPosition:w.ClaimPosition+len(w.Claim)], w.Claim) {
		return fmt.Errorf("claim %q not found at claimed position %d in DateB64", w.Claim, w.ClaimPosition)
	}
	...
	// Decode DateB64 to verify it contains a valid date at DatePosition
	dateJSON, err := base64.RawURLEncoding.DecodeString(w.DateB64)
	if err != nil {
		return fmt.Errorf("failed to decode DateB64: %w", err)
	}
	...
	// Validate DatePosition
	if w.DatePosition < 0 || w.DatePosition+DateSize > len(dateJSON) {
		return fmt.Errorf("DatePosition out of range: %d (decoded DateB64 length: %d)",
			w.DatePosition, len(dateJSON))
	}
...
func (api *API) Parse(publicInputJSON, privateInputJSON []byte) (frontend.Circuit, error) {
	...
	data, err := ExtractData(privateInput.Payload, publicInput.ThresholdDate, publicInput.Claim)
	if err != nil {
		return nil, fmt.Errorf("failed to parse the data: %w", err)
	}
...
type Over18Payload struct {
	...
	DateIndex     int
	ThresholdDate string
	Claim         string // base64 encoded claim
	ClaimSize     int
	ClaimPosition int
}

// ExtractData extracts the relevant date from the payload
func ExtractData(payloadB64 string, thresholdDate string, claim string) (*Over18Payload, error) {
	...
	birthdate, err := base64.RawURLEncoding.DecodeString(dateB64)
	if err != nil {
		return nil, fmt.Errorf("failed to decode birthdate")
	}

	dateIndex := strings.Index(string(birthdate), payloadMap[claim].(string))
	if dateIndex == -1 {
		return nil, fmt.Errorf("failed to get date index")
	}

	claimStart := strings.Index(string(birthdate), claim)
	if claimStart == -1 {
		return nil, fmt.Errorf("failed to get claim index")
	}
